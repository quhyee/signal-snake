...
test('buildAnnouncement can describe board-tap resume guidance when configured', () => {
  const message = buildAnnouncement('paused', 20, 80, { pauseResumeMode: 'board' });

  assert.equal(
    message,
    '状态：暂停中。点击棋盘继续游戏，或按“重新开始”立即开新局。当前分数 20，最高分 80。',
  );
});
...
test('getOverlayCopy returns Chinese overlay text for each visible state', () => {
  ...
  assert.deepEqual(getOverlayCopy('paused', { pauseResumeMode: 'board' }), {
    title: '游戏暂停',
